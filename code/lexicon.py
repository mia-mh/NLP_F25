...
def problex_lexicon(corpus: TaggedCorpus) -> torch.Tensor:
    ...
    PART 3 IMPLEMENTATION: Frequency-based embeddings
    """
    
    V = len(corpus.vocab)
    k = len(corpus.tagset)
    
    log.info(f"Creating frequency-based embeddings: vocab={V}, tags={k}")
    
    # Initialize counts
    word_counts = torch.zeros(V, dtype=torch.float32)
    word_tag_counts = torch.zeros(V, k, dtype=torch.float32)
    total_tokens = 0
    
    # Count word frequencies and tag co-occurrences from corpus
    for sentence in corpus:
        for word, tag in sentence:
            wid = corpus.vocab.index(word)
            tid = corpus.tagset.index(tag)
            
            if wid is not None and tid is not None:
                word_counts[wid] += 1
                word_tag_counts[wid, tid] += 1
                total_tokens += 1
    
    log.info(f"Processed {total_tokens} tokens from {len(corpus)} sentences")
    
    # Create feature matrix: [V, k+1]
    # Columns 0 to k-1: log(p(t|w)) for each tag
    # Column k: log(p(w)) - FINALLY!
    features = torch.zeros(V, k + 1, dtype=torch.float32)
    
    # Columns 0 to k-1: log p(t|w) with add-1 smoothing
    # p(t|w) = (count(w,t) + 1) / (count(w) + k)
    for wid in range(V):
        if word_counts[wid] > 0:
            # Smoothed conditional probabilities
            tag_probs = (word_tag_counts[wid] + 1.0) / (word_counts[wid] + k)
            features[wid, :k] = torch.log(tag_probs)
        else:
            # Unseen word: uniform distribution over tags
            uniform_prob = 1.0 / k
            features[wid, :k] = torch.log(torch.full((k,), uniform_prob))
    
    # Column k: log p(w) with add-1 smoothing - LAST!
    # p(w) = (count(w) + 1) / (total_tokens + V)
    features[:, k] = torch.log((word_counts + 1.0) / (total_tokens + V))
    
    log.info(f"Created frequency-based features with shape {features.shape}")
    log.info(f"  Columns 0-{k-1}: log p(t|w) for each tag")
    log.info(f"  Column {k}: log p(w)")
...
def affixes_lexicon(corpus: TaggedCorpus,
    ...
    EXTRA CREDIT: Not required for basic homework
    """
